...
const getNumberOfAppearencesInOrderItems = (req, callback) => {
  pool.connect((err, client, release) => {
    ...
    const getOrderByClientName = (req, callback) => {
      pool.connect((err, client, release) => {
        if (err) {
          return callback('Error acquiring client because of ' + err)
        }
        client.query('select * from orders where clientname = ' + '\'' + req.clientname + '\'', (err, result) => {
          release()
          if (err) {
            return callback('Error executing query : ' + err)
          }
    ...
    const getOrdersWhereTotalIsOver = (req,callback) => {
      pool.connect((err, client, release) => {
        if (err) {
          return callback('Error acquiring client because of ' + err)
        }
        client.query('select * from orders where total > ' + '\'' + req.total + '\'', (err, result) => {
          release()
          if (err) {
            return callback('Error executing query : ' + err)
          }
    ...
    const getOrdersWhereIsThisItem = (req,callback) => {
      pool.connect((err, client, release) => {
        if (err) {
          return callback('Error acquiring client because of ' + err)
        }
    ...
    module.exports = {
      ...
      getOrdersWhereTotalIsOver,
      getOrdersWhereIsThisItem
    }